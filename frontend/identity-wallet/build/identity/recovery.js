...
exports.WeakPassphraseError = exports.InvalidMnemonicError = exports.MIN_PASSPHRASE_LENGTH = void 0;
exports.assertValidMnemonic = assertValidMnemonic;
exports.sealMnemonic = sealMnemonic;
 ...
 */
exports.MIN_PASSPHRASE_LENGTH = 12;
...
class WeakPassphraseError extends Error {
    constructor() {
        super(`WeakPassphraseError: the backup passphrase must be at least ${exports.MIN_PASSPHRASE_LENGTH} ` +
            "characters — this file is meant to be stored off the device, so it must survive being copied");
        this.name = "WeakPassphraseError";
    }
}
exports.WeakPassphraseError = WeakPassphraseError;
/**
 ...
 */
function assertValidMnemonic(phrase) {
    const normalised = phrase.trim().replace(/\s+/g, " ").toLowerCase();
    if (normalised.length === 0)
        throw new InvalidMnemonicError("empty phrase");
    const words = normalised.split(" ");
    if (words.length !== 12 && words.length !== 24) {
        throw new InvalidMnemonicError(`expected 12 or 24 words, got ${words.length} — this wallet generates 24`);
    }
    // Mnemonic.fromPhrase validates the wordlist AND the checksum; it throws on either.
    try {
        ethers_1.Mnemonic.fromPhrase(normalised);
    }
    catch {
        throw new InvalidMnemonicError("the phrase is not valid BIP39 — a word is misspelled, out of order, or mistyped " +
...
function assertUsablePassphrase(passphrase) {
    if (passphrase.length < exports.MIN_PASSPHRASE_LENGTH)
 ...
 */
async function sealMnemonic(mnemonic, passphrase) {
    const phrase = assertValidMnemonic(mnemonic);
    assertUsablePassphrase(passphrase);
    const json = JSON.parse(await ethers_1.HDNodeWallet.fromPhrase(phrase).encrypt(passphrase));
    delete json.address;
    delete json.id;
    if (json["x-ethers"]) {
 ...
 */
async function openBackup(backupJson, passphrase) {
    let restored;
    try {
        restored = await ethers_1.Wallet.fromEncryptedJson(backupJson, passphrase);
    }
    catch (e) {
        throw new InvalidMnemonicError("the backup could not be decrypted — wrong passphrase, or the file is corrupt " +
            `(${e.message})`);
    }
    ...
    const phrase = restored.mnemonic?.phrase;
    if (!phrase) {
        throw new InvalidMnemonicError("this keystore contains a private key but NO recovery phrase, so it cannot restore an " +
