...
library PoseidonT4Inline {
  ...
  function hash(uint[3] memory inp) internal pure returns (uint result) {
    assembly {
        let _s0 := mload(0x80)
        let _s1 := mload(0xa0)
        let _s2 := mload(0xc0)
...

      {
        ...
        result := mod(mod(add(add(add(mulmod(state0, M00, F), mulmod(state1, M10, F)), mulmod(state2, M20, F)), mulmod(state3, M30, F)), F), F)
        mstore(0x80, _s0)
        mstore(0xa0, _s1)
        mstore(0xc0, _s2)
