...
| | on | referenced | why it is still there |
|---|---|---|---|
| **`AggregationHonkVerifier`** | 5.1.0 | **yes, 2 refs** | regenerating its fixture needs **~21.7 GB** |
| `verifiers2/noir/*` (79 passport) | 5.1.0 | **no** | generated by `codegen-passport-verifiers.sh`, nothing wires verifiers by address yet, so **no test would catch a mistake either way** |

**ONE referenced artifact.** And it is the flat aggregator, which BOTH surviving settlement paths
supersede - the tree does the same batch at 2.11 GB and less gas, the fold+root does it at any N.
**So the answer is to retire it, not spend 21.7 GB upgrading it.**
...
- [ ] **Retire `aggregate_withdrawals` and `AggregationHonkVerifier`** rather than regenerate them.
      That is a deletion on the money path and needs its own turn: check the 2 references, keep
      `AggregationProofOnChain.t.sol`'s measurement in TODO before deleting the test, and confirm
      nothing in `../ibiza`'s four pinned signatures depends on it.
- [ ] **The 79 passport verifiers are untested, which matters more than their bb version.** Nothing
      wires verifiers by address, so a wrong one would be caught by nothing. Regenerating them on 6.0
      without first wiring and testing them just moves untested artifacts.
