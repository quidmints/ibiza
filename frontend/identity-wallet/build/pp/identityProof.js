...
exports.STATUS_CLEAN = exports.IDENTITY_TREE_DEPTH = void 0;
exports.fetchIdentityWitness = fetchIdentityWitness;
exports.isIdentityRootStillValid = isIdentityRootStillValid;
 ...
 */
async function fetchIdentityWitness(registryAddress, runner, commitment) {
    const registry = new ethers_1.Contract(registryAddress, IDENTITY_REGISTRY_ABI, runner);
    const key = "0x" + commitment.toString(16).padStart(64, "0");
    if (!(await registry.registered(key))) {
        throw new Error("fetchIdentityWitness: this commitment is NOT registered. Post the escrow envelope " +
            "(escrow_envelope) before attempting to withdraw - registration is what puts the identity " +
            "in the tree at all.");
    }
    const status = await registry.statusOf(key);
    if (BigInt(status) !== exports.STATUS_CLEAN) {
        throw new Error(`fetchIdentityWitness: this identity is REVOKED under predicate ${status}. No withdrawal ` +
    ...
    if (!p.existence) {
        ...
        throw new Error("fetchIdentityWitness: registry reports the key as ABSENT while registered() said yes");
    }
    if (BigInt(p.value) !== exports.STATUS_CLEAN) {
        // Same class: the tree's leaf value and `statusOf` disagree.
        throw new Error(`fetchIdentityWitness: leaf value is ${p.value} while statusOf() reported clean`);
    }
    const siblings = p.siblings.map((s) => BigInt(s));
    if (siblings.length > exports.IDENTITY_TREE_DEPTH) {
        throw new Error(`fetchIdentityWitness: registry depth ${siblings.length} exceeds the circuit's ` +
            `${exports.IDENTITY_TREE_DEPTH}. The tree has outgrown the circuit; both must be regenerated together.`);
    }
    while (siblings.length < exports.IDENTITY_TREE_DEPTH)
        siblings.push(0n);
 ...
 */
async function isIdentityRootStillValid(registryAddress, runner, root) {
    const registry = new ethers_1.Contract(registryAddress, IDENTITY_REGISTRY_ABI, runner);
