...
TARGETS=(
  ...
  # The first passport-registration verifier this repo can PROVE against (task 24). The 76
  # `passport/verifiers2/noir/NoirRegisterIdentity_*.sol` are rarimo's, generated from 76
  # parameterised profiles that do not exist here - there is no generator, so they cannot be
  # regenerated and nothing references them. Of our five identity circuits this is the only one with
  # a committed witness, so it is the only one whose verifier can be self-checked rather than merely
  # emitted; the rest wait on a real document (task 6). 18,429 bytes runtime, 6,147 under EIP-170.
  "register_identity_light_td1:${CONTRACTS_DIR}/passport/verifiers/RegisterIdentityLightHonkVerifier.sol:RegisterIdentityLightHonkVerifier:3"
...
if [ -d "${WALLET_CIRCUITS}" ]; then
  ...
  for target in "${TARGETS[@]}"; do
    ...
    src="${CIRCUITS_DIR}/${circuit}/target/$(sed -n 's/^name *= *"\(.*\)"/\1/p' "${CIRCUITS_DIR}/${circuit}/Nargo.toml" | head -1).json"
    if [ -f "${src}" ]; then
