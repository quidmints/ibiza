...
library PoseidonT5Inline {
  ...
  function hash(uint[4] memory inp) internal pure returns (uint result) {
    assembly {
        let _s0 := mload(0x80)
        let _s1 := mload(0xa0)
        let _s2 := mload(0xc0)
        let _s3 := mload(0xe0)
        mstore(0x80, mload(add(inp, 0x0)))
        mstore(0xa0, mload(add(inp, 0x20)))
        mstore(0xc0, mload(add(inp, 0x40)))
        mstore(0xe0, mload(add(inp, 0x60)))
...

      {
        ...
        result := mod(add(mod(add(add(add(mulmod(state0, M00, F), mulmod(state1, M10, F)), mulmod(state2, M20, F)), mulmod(state3, M30, F)), F), mulmod(state4, M40, F)), F)
        mstore(0x80, _s0)
        mstore(0xa0, _s1)
        mstore(0xc0, _s2)
        mstore(0xe0, _s3)
